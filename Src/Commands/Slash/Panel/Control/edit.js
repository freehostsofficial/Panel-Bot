...
module.exports = {
  ...
  async execute(client, interaction) {
    ...
    if (!newUrlInput && !newApiKey) {
      return interaction.editReply('❌ Mission parameters missing: Specify either a new URL or API Key.');
    }
    ...
    if (!panel) {
      return interaction.editReply(`❌ Panel target **${name}** not identified in registry.`);
    }
    ...
    if (newUrlInput) {
      ...
      if (!cleanUrl.startsWith('http')) {
        return interaction.editReply('❌ Protocol violation: URL must utilize `http://` or `https://`.');
      }
      panel.url = cleanUrl;

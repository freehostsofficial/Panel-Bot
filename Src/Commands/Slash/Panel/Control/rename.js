...
module.exports = {
  ...
  async execute(client, interaction) {
    ...
    if (!panel) {
      return interaction.editReply(`❌ Legacy identifier **${currentName}** not found in registry.`);
    }

    const duplicate = userData.panels.some(p => p.name.toLowerCase() === newName.toLowerCase());
    if (duplicate) {
      return interaction.editReply(`❌ Registry conflict: Identifier **${newName}** is already allocated.`);
    }

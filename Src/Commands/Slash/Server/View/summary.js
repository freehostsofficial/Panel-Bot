...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        try {
            ...
            if (servers.length === 0) return interaction.editReply("❌ No servers found across your panels.");
            ...
            const serverSamples = await Promise.all(
                servers.slice(0, sampleSize).map(async (s) => {
                    try {
                        ...
                        if (info.resources.current_state === 'running') running++;
                        else if (info.resources.current_state === 'offline') offline++;
                        else other++;
